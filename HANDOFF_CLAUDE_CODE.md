...
- `api/migrations/`: fortlaufend nummeriert, aktuell bis `044_user_birthday_full_date.sql`. Wenn ein CHECK-Constraint erweitert **oder eine Spalte entfernt** werden muss (SQLite kann beides nicht zuverlässig per ALTER TABLE): Tabelle neu aufbauen wie in `007_household_invites.sql`/`032_calendar_childcare_type.sql`/`041_feed_items_poll_marketplace_types.sql`/`044_user_birthday_full_date.sql` (`PRAGMA foreign_keys = OFF`, neue Tabelle mit `_new`-Suffix, Daten kopieren, alte droppen, umbenennen, `PRAGMA foreign_keys = ON`). **Falle, in die wir schon reingelaufen sind (1):** `feed_items.type` hat einen CHECK-Constraint aus einer alten PRD-Fassung, der nicht mit `FeedItem::MVP_TYPES` übereinstimmt - vor jedem neuen Feed-Post-Typ prüfen, ob er im CHECK-Constraint der Tabelle steht (`grep "CHECK (type IN" api/migrations/*.sql`), sonst gibt's einen 500er erst beim tatsächlichen INSERT, nicht schon vorher. **Falle (2):** `ALTER TABLE ... DROP COLUMN` **nie benutzen** - das SQLite des Hosters ist < 3.35 und kennt es nicht. Migration 044 lief deswegen beim ersten Deploy mittendrin auf einen Fehler (nach dem `ADD COLUMN`, vor dem `_migrations`-Eintrag) → jeder folgende Request wiederholte die Migration, scheiterte an "duplicate column" und die **komplette API war down (leerer 500er)**. Spalten immer per Tabellen-Rebuild loswerden, und den Rebuild idempotent gegen einen evtl. schon halb angewandten Zustand schreiben (Spalte im `CREATE`, aber nicht im `INSERT ... SELECT` referenzieren).
...
- **Kalender-Termine** (`/kalender`, "+ Termin"): Wiederholung jetzt auch **jährlich** (zusätzlich zu täglich/wöchentlich/monatlich, für Geburtstage o.ä.), Sichtbarkeit nur noch **Nachbarschaft** oder **Privat** wählbar (kein "Öffentlich" mehr - macht ohne Gäste-Rolle keinen Sinn, siehe Sensible Stellen). Kategorie **"Event" ist im Erstell-Formular bewusst nicht mehr wählbar** - eine reine Kalendernotiz kann nicht auf "dabei/vielleicht/nicht dabei" reagieren, ohne RSVP zu duplizieren; dafür gibt's die echten Events (siehe unten), die automatisch mit im Kalender auftauchen. Bestehende alte "Event"-Kalendernotizen (vor diesem Umbau) bleiben lesbar/editierbar, nur keine neuen mehr.
...
- **Geburtstage**: `users.birthday` (volles Datum `YYYY-MM-DD` mit Jahr, seit Migration 044), self-service über ein natives `date`-Feld in den Profil-Einstellungen. Alter wird daraus berechnet und angezeigt ("wird 42"). Kinder-Geburtstage aus dem schon vorhandenen `children.birthdate`-Feld - **bewusst für ALLE Nachbarn sichtbar**, NICHT über die `children`-Sichtbarkeitseinstellung gefiltert (sonst kein Mehrwert). Dashboard zeigt ein Banner "Heute Geburtstag: ..." - `DashboardController::todaysBirthdays()` kombiniert `User::todaysBirthdays()` + `Child::todaysBirthdays()` (Match über `strftime('%m-%d', ...)`). Im **Kalender** tauchen Geburtstage als synthetische, jährlich wiederkehrende Einträge auf (`source='birthday'`, `canManage=false`, read-only): `CalendarController::birthdayItems()` baut sie on-the-fly aus `User::withBirthday()` + `Child::withBirthday()` über den `RecurrenceExpander` - **keine eigene Tabelle, keine Datendopplung**, immer synchron mit Profil/Kinder-Angaben. Pflege ausschließlich dort, nicht im Kalender.
  - Migration 044 baut `users` neu auf (Muster wie 041/043) statt `ALTER TABLE ... DROP COLUMN` (`birthday_month`/`birthday_day`) - **DROP COLUMN unterstützt das SQLite des Hosters (< 3.35) nicht** und hat die Migration beim ersten Deploy mittendrin abgebrochen (Spalte `birthday` war schon angelegt, `_migrations`-Eintrag fehlte → jeder Request lief auf "duplicate column: birthday" und die ganze API war down). Der Rebuild ist idempotent gegen genau diesen Teil-Zustand: `birthday` entsteht aus dem `CREATE` und wird im `INSERT ... SELECT` nicht referenziert.
- **Rezepte** (`/rezepte`, `/rezepte/:id`) **+ Einkaufsliste** (`/einkaufsliste`), Feature-Flag `recipes`, Migration 045. Bewusst nur der kleine Teil eines größeren Vorhabens - **Wochenplan und Vorrat (mit Zutaten-Abgleich) sind explizit NICHT gebaut**, das sind eigene, größere Vorhaben (siehe Roadmap). Rezepte sind eine **eigene Tabelle** (`recipes`/`recipe_tags`/`recipe_comments`/`recipe_reactions`), kein Feed-Post-Typ - die Datenform (Zutaten/Zubereitung/Portionen/Dauer/Schwierigkeit) passt nicht zum generischen `message`-Feld. **Immer nachbarschaftsweit sichtbar** (keine `visibility`-Spalte, anders als beim Feed) - Kommentare/Reaktionen (Herz) 1:1 wie bei Feed-Posts, nur Bearbeiten/Löschen bleibt dem erstellenden Haushalt bzw. Admin vorbehalten (`RecipeController::canManage()`). Feste Tag-Auswahl (`RecipeController::TAGS`) statt Freitext für sinnvolle Filter-Pillen. Die Einkaufsliste ist bewusst **privat pro Haushalt** (wie Kinder/Haustiere), kein Rezept-Bezug - reine manuelle Checkliste, kein Auto-Abgleich.
...
Sichtbarkeits-Level (Öffentlich / Nur Nachbarn / Privat) sind fachlich wichtig, betreffen Status, Urlaub, Kinder, Events, Nachbarschaftsverzeichnis. Neue Features mit Sichtbarkeit (z. B. Terminfindung, Events) folgen demselben `public`/`neighbors`-Muster wie `EventController::isVisible()`. Ausnahme: Kalender-Termine (`calendar_entries`) bieten seit dem Umbau bewusst nur noch Nachbarschaft/Privat an (kein Öffentlich) - alte öffentliche Einträge bleiben aber lesbar, das Schema erlaubt den Wert weiterhin.
...
4. **Wochenplan** (Rezepte einem Tag zuordnen) und **Vorrat** (Zutaten-Abgleich Rezepte ↔ Einkaufsliste, "X Zutaten fehlen") - bewusst zurückgestellter, größerer Teil des Rezepte-Vorhabens (siehe Funktionsstand). Braucht eigenes Mengen-/Einheiten-Handling (g/Stück/Bund) und Zutaten-Namens-Matching - eigenes Arbeitspaket, kein Anhängsel an die bestehenden Rezepte-Tabellen.
