...
- `api/migrations/`: fortlaufend nummeriert, aktuell bis `038_conversations_user_level.sql`. Wenn ein CHECK-Constraint erweitert werden muss (SQLite kann das nicht per ALTER TABLE): Tabelle neu aufbauen wie in `007_household_invites.sql`/`032_calendar_childcare_type.sql` (`PRAGMA foreign_keys = OFF`, neue Tabelle mit `_new`-Suffix, Daten kopieren, alte droppen, umbenennen, `PRAGMA foreign_keys = ON`).
